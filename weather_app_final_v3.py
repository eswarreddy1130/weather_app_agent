...
# 🌈 Custom CSS for Glassmorphism + Mobile Feel
st.markdown("""
    <style>
    body {
...
st.markdown("<p style='text-align:center;'>Your smart and beautiful weather assistant</p>", unsafe_allow_html=True)
...
if city:
    with st.spinner("Fetching weather data..."):
        ...
        if "results" in geo_res and len(geo_res["results"]) > 0:
            ...
            # 🖼️ Background image (Unsplash API - no key needed for this static link style)
            bg_url = f"https://source.unsplash.com/1600x900/?{city},city,skyline"
            st.markdown(f"""
                <style>
                .stApp {{
            ...
            for t, temp, code in zip(df["Time"], df["Temperature (°C)"], weather_codes[:24]):
                ...
                st.markdown(
                    f"""
                    <div style="padding:8px; margin:5px; background:rgba(255,255,255,0.25);
